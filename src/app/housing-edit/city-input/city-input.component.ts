...
export class CityInputComponent implements OnInit {
  ...
  updateCityList() {
    ...
    if (this.previousSelectedCityId != null)
      this.cityList[this.previousSelectedCityId!].selected = false;
    this.cityList[this.selectedCityId!].selected = true;

    // scroll cityList
    let selectedY = 0;
    let selectedHeight = (
      document.getElementById('city-list')?.children[
        this.selectedCityId!
      ] as HTMLElement
    ).offsetHeight;
    let cityListHeight = (document.getElementById('city-list') as HTMLElement)
      .offsetHeight;
    ...
    for (let i = 0; i < this.selectedCityId!; i++) {
      selectedY += (
        document.getElementById('city-list')?.children[i] as HTMLElement
      ).offsetHeight;
    }
    ...
    if (
      selectedY -
        selectedHeight -
        (document.getElementById('city-list') as HTMLElement).scrollTop <
      0
    ) {
      // scroll up
      ...
      document
        ...
        ?.scroll(0, selectedY - cityListHeight + selectedHeight);
    }
  ...
  closeList() {
    ...
    if (this.selectedCityId)
      this.cityList[this.selectedCityId!].selected = false;
    this.selectedCityId = null;
    this.previousSelectedCityId = null;
